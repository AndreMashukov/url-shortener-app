...
Step 2. Verify the rule's target permission.
  ...
  Confirm Arn is the SQS ARN (note: boto3 in this account masks the ARN, so the
  field will show "arn:aws:sqs:ap-southeast-1:579273601730:***"; the literal
  string "***" is what EventBridge sees as the target ARN and it works
  because that is the masked form of the real ARN, see memory).
...
All event-driven flows in the monorepo are blocked. The redirect-bff lean view
materialization (which is supposed to be CDC via the bus) is currently
happening via some other path. Wait, that cannot be right. Let me re-examine
that, because the lean table updates are happening even though the listener
queue is empty. Actually re-examining the data: the smoke test that produced
a 302 in 1 to 2 tries for the freshly created mapping. The listener is
supposed to read from the SQS queue, and the SQS queue is empty. So either
the lean view is being updated by a different path, or my interpretation of
the 302 latency is wrong. To be confirmed.

If the lean view is being updated by some other path (such as the DDB stream
trigger writing directly to the lean table, or a backfill), then the SQS
listener in redirect-bff is in fact not the path that materializes the lean
view, and the empty queue is the correct steady state. In that case the
analytics-bff is the only consumer affected by this issue. This needs to be
disambiguated before we can scope the impact properly.
